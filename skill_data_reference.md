...
発動時に実行される Minecraft コマンド of 配列です。実行者は発動したプレイヤーになり、`@s` で自身を対象にできます。
* 例:
  ```javascript
  commands: [
...
## 8. 属性・耐性システム (Elements & Resistances)
本作では、8つの属性（無・炎・水・土・雷・悪・光・風）が定義されており、モブへの属性設定およびダメージへの属性付与を通じて、弱点によるダメージ増幅、魔法・物理耐性を適用させることができます。

### 属性相性表 (循環関係)
ダメージ属性（攻撃）とモブ自身の属性（防御）が以下の関係に該当する場合、**弱点（WEAKNESS）となり、ダメージが一律 1.5 倍**になります。耐性による半減は適用されません。

* **炎** は **風** に強い (炎ダメージ → 風モブ = 1.5倍)
* **風** は **土** に強い (風ダメージ → 土モブ = 1.5倍)
* **土** は **雷** に強い (土ダメージ → 雷モブ = 1.5倍)
* **雷** は **水** に強い (雷ダメージ → 水モブ = 1.5倍)
* **水** は **炎** に強い (水ダメージ → 炎モブ = 1.5倍)
* **光** と **悪** は互いに強い (光 ⇄ 悪 = 1.5倍)
* **無** は相性関係なし

### 設定方法

#### 1. ダメージに属性を付与する (`entityPatch.damage`)
アクション（`commands` や `script` 等）から `entityPatch.damage` を呼ぶ際、`options.element` に属性名（日本語）を指定します。

* 例（スクリプト内での実行）:
  ```javascript
  // ゾンビに炎属性の魔法ダメージ10を与える
  entityPatch.damage(entity, 10, { 
      damageType: "magic", 
      element: "炎",
      damagerId: player.id 
  });
...
#### 2. モブに属性・耐性を設定する (`mobdata.js`)
モブの定義オブジェクトに、以下のプロパティを追加します。
* `element` (String): モブ自身の属性（`"無"`, `"炎"`, `"水"`, `"土"`, `"雷"`, `"悪"`, `"光"`, `"風"`）
* `physical_resistance` (Number): 物理ダメージに対する倍率（例: `0.5` なら物理ダメージ半減、`1.0` は等倍）
* `magic_resistance` (Number): 魔法ダメージに対する倍率（例: `0.8` なら魔法ダメージ20%カット）
...
* 例:
  ...
  "minecraft:zombie": {
      "name": "ゾンビ",
      "element": "土",            // 風が弱点になる
      "physical_resistance": 1.0, // 物理等倍
      "magic_resistance": 0.8,    // 魔法20%カット
      // ...他のステータス
  }
  ```
